...
class TaoBao(object):
    def __init__(self):
        ...
        self.url = "https://detail.tmall.com/item.htm?id=557200845972"
        self.proxies = {'http': 'http://16SBYYUY:658666@n10.t.16yun.cn:6442', 'https': 'http://16SBYYUY:658666@n10.t.16yun.cn:6442'}
    ...
    def test(self):
        ...
        for i in range(40):
            name = 'taobao%s.jpg' % i
            print name
            res = self.srequest.get(self.url, headers=self.headers, proxies=self.proxies)
            time.sleep(0.3)
            ...
            url = "http://gd2.alicdn.com/imgextra/i2/54507609/TB2Hd3SXXkoBKNjSZFEXXbrEVXa_!!54507609.jpg"
            resp = self.srequest.get(url, headers=self.headers, proxies={'http': 'http://16SBYYUY:658666@n10.t.16yun.cn:6442'})
            print res.status_code
            print resp.status_code
            if res.status_code == 200:
                ok_count += 1
            time.sleep(0)
            res = self.srequest.get("http://2017.ip138.com/ic.asp", headers=self.headers, proxies={'https': 'http://16SBYYUY:658666@n10.t.16yun.cn:6442'})
            print res.status_code
            resp = res.content
