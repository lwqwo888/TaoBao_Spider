...
class Taobao_Img(object):
    ...
    def turn_img(self, id, category_name, dir_name):
        ...
        for true_url in cover_img_list:
            ...
            with open("img_url.txt", 'a') as f:
                f.write("https:" + true_url + '\n')
                # print "https:" + true_url
    ...
    def detail_img(self, id, category_name, dir_name):
        ...
        res = ""
        self.resp_code = ""
        proxy = {'http': 'http://16SBYYUY:658666@n10.t.16yun.cn:6442'}
        self.change_ua()
        
        # self.srequest = requests.Session()
        # time.sleep(0.2)
        try:
            response = self.srequest.get(desc_url, headers=self.headers, proxies=proxy)
            self.resp_code = response.status_code
            if self.resp_code == 200:
            ...
            self.log(str(e))
            with open('request.log', 'a') as f:
                f.write('%s [代理]请求失败(%s) %s - %s\n' % (desc_url, self.resp_code, id, time.ctime()))
            # # global NETWORK_STATUS
            NETWORK_STATUS = False  # 请求超时改变状态
        ...
        # if not length:
        #     self.detail_img(id, category_name, dir_name)
        print "准备下载细节图"
